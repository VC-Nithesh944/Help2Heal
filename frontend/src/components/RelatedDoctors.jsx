...
const RelatedDoctors = ({ speciality, docId }) => {
  //destructered both props passed in appointment page
  const { doctors } = useContext(AppContext);
  const [relDoc, setRelDocs] = useState([]);
  const navigate = useNavigate();

  //We should show the related doctors excluding the on we are already on(The current Doctors Appointment page : doc._id !== docId)
  useEffect(() => {
        ...
        {relDoc.slice(0, 5).map((item, index) => (
          <div
            ...
            <div className="p-4">
              ...
              <p className="text-grey-900 text-sm">{item.speciality}</p>
            </div>
          </div>
        ))}
      </div>
